...
const Gallery = () => {
  ...
  return (
    <div className="">
      ...
      <button className="flex justify-center hover:bg-brand_color2 transition-all duration-150  items-center text-xs tracking-wider px-[10px] h-[40px] bg-gray-800">
        VIEW MORE
      </button>
    </div>
      <div className="grid grid-cols-3 lg:grid-cols-3 gap-1 lg:gap-4 mt-4">
        {images.map((img, index) => (
          <div key={index} className="relative group overflow-hidden shadow-lg">
            <img src={img} alt="Gallery Item" className="w-full h-[150px] lg:h-40 object-cover transition-transform duration-300 group-hover:scale-110" />
