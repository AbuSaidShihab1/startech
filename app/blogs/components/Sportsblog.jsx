    ...
    <section className="grid grid-cols-1 lg:grid-cols-3 gap-4 py-4">
      {articles.slice(0, 2).map((article, index) => (
        <div key={index} className="relative group overflow-hidden">
          ...
          />
          <div className="absolute top-2 left-2 bg-brand_color2 text-white text-xs font-[600] px-2 py-1 rounded">
            SPORTS
          </div>
          <div className="absolute bottom-0 bg-gradient-to-t from-black via-transparent to-transparent w-full p-4">
            <h3 className="text-white font-[600] text-[14px] xl:text-[16px]">{article.title}</h3>
            <p className="text-gray-300 text-[13px]">Sora Blogging Tips • Dec 05, 2015</p>
          </div>
        </div>
      ))}
        ...
        {articles.slice(2).map((article, index) => (
          <div key={index} className="relative group overflow-hidden ">
            <img
              ...
              width={500}
              height={150}
              className="w-full h-[200px] lg:h-32 object-cover transition-transform duration-300 group-hover:scale-105"
            />
              <div className="absolute top-2 left-2 bg-brand_color2 text-white text-xs font-[600] px-2 py-1 rounded">
            SPORTS
          </div>
